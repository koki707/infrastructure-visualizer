...
export function GlossaryIndexPage({ onNavigate }: { onNavigate: Navigate }) {
  ...
  return <PageContainer>
    <SectionTitle title="用語集" lead="シミュレーション中に登場する用語を、通信の階層や役割から探せます。正式名称・役割・関連する仕組みも確認できます。" />
...
export function GlossaryDetailPage({ term, onNavigate }: { term: GlossaryTerm; onNavigate: Navigate }) {
  ...
  return <PageContainer>
    ...
    <section className="mt-10 max-w-3xl space-y-5">
      ...
      <article className="panel p-6"><h2 className="text-lg font-bold text-slate-900">シミュレーションとの関係</h2><p className="mt-3 text-sm leading-7 text-slate-600"><GlossaryText text="この用語は、URLアクセスの流れや機器・データ構造の探索で、他の仕組みと組み合わせて登場します。通信の全体像に戻り、関連する場面を確認してみましょう。" onOpenTerm={(termId) => onNavigate(`/glossary/${termId}`)} /></p><button type="button" onClick={() => onNavigate('/visualizer')} className="mt-5 rounded-lg bg-cyan-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-cyan-700">シミュレーションに戻る</button></article>
