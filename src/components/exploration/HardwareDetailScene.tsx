...
function MemoryBlocks({ world, selectedId, onSelect }: { world: ExplorationWorld; selectedId: string | null; onSelect: (item: ExplorationItem) => void }) {
  ...
  return <>
    ...
    <ClickTarget item={items.read} position={[4.55, 1.5, 0]} selected={selectedId === 'read'} onSelect={onSelect}><mesh><boxGeometry args={[1.35, .68, .3]} /><meshStandardMaterial color="#047857" emissive="#34d399" emissiveIntensity={.26} /></mesh><Text position={[0, .02, .19]} fontSize={.14} color="white" anchorX="center">CPUへ読み出し</Text></ClickTarget>
    <ClickTarget item={items.write} position={[4.55, -1.4, 0]} selected={selectedId === 'write'} onSelect={onSelect}><mesh><boxGeometry args={[1.35, .68, .3]} /><meshStandardMaterial color="#be185d" emissive="#fb7185" emissiveIntensity={.24} /></mesh><Text position={[0, .02, .19]} fontSize={.14} color="white" anchorX="center">CPUから書き込み</Text></ClickTarget>
