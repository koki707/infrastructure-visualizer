...
function SwitchInterior({ world, selectedId, onSelect }: { world: ExplorationWorld; selectedId: string | null; onSelect: (item: ExplorationItem) => void }) {
  ...
  return <>
    <Chassis label="LAN SWITCH · L2 FRAME FORWARDING" />
    <DevicePart item={items['switch-in']} position={[-5.55, -.78, 0]} selected={selectedId === 'switch-in'} onSelect={onSelect}><PortBank label="port 2" /></DevicePart>
    <DevicePart item={items['frame-check']} position={[-3.85, -.78, 0]} selected={selectedId === 'frame-check'} onSelect={onSelect}><Display lines={['FRAME CHECK', 'FCS / TYPE']} /></DevicePart>
    <DevicePart item={items['destination-mac']} position={[-1.8, .62, 0]} selected={selectedId === 'destination-mac'} onSelect={onSelect}><Display lines={['DST MAC', '3C:52:82:··']} /></DevicePart>
    <DevicePart item={items['mac-table']} position={[.3, .62, 0]} selected={selectedId === 'mac-table'} onSelect={onSelect}><Display lines={['MAC TABLE', '3C:52 → p4', 'A4:19 → p7']} /></DevicePart>
    <DevicePart item={items['switch-fabric']} position={[2.35, .62, 0]} selected={selectedId === 'switch-fabric'} onSelect={onSelect}><group><mesh rotation={[Math.PI / 2, 0, 0]}><cylinderGeometry args={[.72, .72, .32, 10]} /><meshStandardMaterial color="#314a5d" metalness={.7} roughness={.33} /></mesh><mesh position={[0, 0, .2]}><circleGeometry args={[.36, 16]} /><meshStandardMaterial color="#b7d7da" emissive="#0f766e" emissiveIntensity={.22} /></mesh><Text position={[0, .03, .23]} fontSize={.13} color="#10202c" anchorX="center">FABRIC</Text></group></DevicePart>
    <DevicePart item={items['egress-queue']} position={[4.25, -.78, 0]} selected={selectedId === 'egress-queue'} onSelect={onSelect}><Display lines={['EGRESS', 'QUEUE']} /></DevicePart>
    <DevicePart item={items['switch-out']} position={[5.7, -.78, 0]} selected={selectedId === 'switch-out'} onSelect={onSelect}><PortBank label="port 4" /></DevicePart>
    <Line points={SWITCH_PATH} color="#7dd3dc" lineWidth={1.5} transparent opacity={.68} />
    <PacketPulse points={SWITCH_PATH} color="#67e8f9" />
    <Text position={[0, -1.63, .2]} fontSize={.15} color="#dbeafe" outlineWidth={.008} outlineColor="#0f172a" anchorX="center">SwitchはEthernet Frameを保ったまま、MACアドレスを見て転送する</Text>
    <ProtocolUnit position={[-3.3, -2.32, .12]} kind="frame" caption="受信 Frame：Ethernetヘッダを参照" />
    <ProtocolUnit position={[3.35, -2.32, .12]} kind="frame" caption="送出 Frame：上位のIP / TCPは開かない" />
  </>
}
...
function RouterInterior({ world, selectedId, onSelect }: { world: ExplorationWorld; selectedId: string | null; onSelect: (item: ExplorationItem) => void }) {
  const items = useMemo(() => itemMap(world), [world])
  return <>
    <Chassis label="ROUTER · L3 FORWARDING" accent="#4a3f31" />
    ...
    <DevicePart item={items['l2-decap']} position={[-3.9, -.78, 0]} selected={selectedId === 'l2-decap'} onSelect={onSelect}><Display lines={['L2 RECEIVE', 'REMOVE ETH']} /></DevicePart>
    <DevicePart item={items['ip-lookup']} position={[-2.05, .62, 0]} selected={selectedId === 'ip-lookup'} onSelect={onSelect}><Display lines={['DST IP', '93.184.216.34']} /></DevicePart>
    <DevicePart item={items.ttl} position={[-.15, .62, 0]} selected={selectedId === 'ttl'} onSelect={onSelect}><Display lines={['TTL', '64 → 63']} /></DevicePart>
    <DevicePart item={items.route} position={[1.85, .62, 0]} selected={selectedId === 'route'} onSelect={onSelect}><Display lines={['ROUTING TABLE', '0.0.0.0/0 → ISP', 'Longest Prefix']} /></DevicePart>
    <DevicePart item={items['next-hop']} position={[3.8, .62, 0]} selected={selectedId === 'next-hop'} onSelect={onSelect}><Display lines={['NEXT HOP', 'ISP Router']} /></DevicePart>
    ...
    <Text position={[0, -1.63, .2]} fontSize={.15} color="#f5e6c4" outlineWidth={.008} outlineColor="#0f172a" anchorX="center">Ethernetだけを外す → IPで経路を決める → 次のリンク用のFrameを作り直す</Text>
    <ProtocolUnit position={[-4.05, -2.32, .12]} kind="frame" caption="受信 Frame" />
    <ProtocolUnit position={[0, -2.32, .12]} kind="packet" caption="Ethernetを外した IP Packet" />
    <ProtocolUnit position={[4.05, -2.32, .12]} kind="frame" caption="次ホップ用の新しい Frame" />
  </>
}
...
function DnsInterior({ world, selectedId, onSelect }: { world: ExplorationWorld; selectedId: string | null; onSelect: (item: ExplorationItem) => void }) {
  const items = useMemo(() => itemMap(world), [world])
  return <>
    <Chassis label="DNS RESOLVER · NAME RESOLUTION" accent="#334d68" />
    ...
    <DevicePart item={items['dns-query-item']} position={[-3.45, -.75, 0]} selected={selectedId === 'dns-query-item'} onSelect={onSelect}><Display lines={['DNS QUERY', 'example.test?']} /></DevicePart>
    <DevicePart item={items['dns-resolver']} position={[-1.2, .45, 0]} selected={selectedId === 'dns-resolver'} onSelect={onSelect}><Display lines={['RESOLVER', 'parse / lookup']} /></DevicePart>
    <DevicePart item={items['dns-cache']} position={[1.1, .45, 0]} selected={selectedId === 'dns-cache'} onSelect={onSelect}><Display lines={['CACHE', 'TTL checked']} /></DevicePart>
    <DevicePart item={items['dns-upstream']} position={[3.45, -.75, 0]} selected={selectedId === 'dns-upstream'} onSelect={onSelect}><Display lines={['UPSTREAM', 'if cache miss']} /></DevicePart>
    <DevicePart item={items['dns-answer-item']} position={[5.45, -.75, 0]} selected={selectedId === 'dns-answer-item'} onSelect={onSelect}><Display lines={['ANSWER', 'A / AAAA']} /></DevicePart>
...
function ServerInterior({ world, selectedId, onSelect }: { world: ExplorationWorld; selectedId: string | null; onSelect: (item: ExplorationItem) => void }) {
  const items = useMemo(() => itemMap(world), [world])
  return <>
    <Chassis label="WEB SERVER · REQUEST PROCESSING" />
    ...
    <DevicePart item={items['physical-in']} position={[-4.05, -.8, 0]} selected={selectedId === 'physical-in'} onSelect={onSelect}><Display lines={['PHY', 'DECODE SIGNAL']} /></DevicePart>
    <DevicePart item={items['ethernet-in']} position={[-2.35, .62, 0]} selected={selectedId === 'ethernet-in'} onSelect={onSelect}><Display lines={['ETHERNET', 'REMOVE FRAME']} /></DevicePart>
    <DevicePart item={items['ip-in']} position={[-.55, .62, 0]} selected={selectedId === 'ip-in'} onSelect={onSelect}><Display lines={['IPv4', 'DST IP CHECK']} /></DevicePart>
    <DevicePart item={items['tcp-in']} position={[1.25, .62, 0]} selected={selectedId === 'tcp-in'} onSelect={onSelect}><Display lines={['TCP', 'PORT 443']} /></DevicePart>
    <DevicePart item={items['web-server']} position={[3.35, .62, 0]} selected={selectedId === 'web-server'} onSelect={onSelect}><Display lines={['WEB SERVER', 'HTTP REQUEST']} /></DevicePart>
    <DevicePart item={items['server-app']} position={[5.35, -.8, 0]} selected={selectedId === 'server-app'} onSelect={onSelect}><Display lines={['APP WORKER', 'HANDLE REQUEST']} /></DevicePart>
    ...
    <Text position={[0, -2.23, .2]} fontSize={.15} color="#dbeafe" outlineWidth={.008} outlineColor="#0f172a" anchorX="center">受信側：Ethernet → IP → TCPの順に外し、HTTP Requestをアプリへ渡す</Text>
    <ProtocolUnit position={[-4.5, -2.95, .12]} kind="frame" caption="Ethernet Frame" />
    <ProtocolUnit position={[-1.55, -2.95, .12]} kind="packet" caption="IP Packet" />
    <ProtocolUnit position={[1.35, -2.95, .12]} kind="segment" caption="TCP Segment" />
    <ProtocolUnit position={[4.45, -2.95, .12]} kind="data" caption="HTTP Request" />
  </>
}
