...
export function DataPacket({ journey }: { journey: PacketJourney }) {
  const ref = useRef<Group>(null)
  useFrame(({ clock }) => { if (ref.current) ref.current.rotation.y = clock.elapsedTime * 2.4 })
  ...
  const curve = useMemo(() => {
    const points: Position[] = journey.nodePath.flatMap((id, index) => {
      ...
      return [NETWORK_NODES.find(node => node.id === id)?.position ?? NETWORK_NODES[0].position]
    })
    const vectors = points.map(point => new Vector3(...point))
    return new CatmullRomCurve3(vectors.length > 1 ? vectors : [new Vector3(...NETWORK_NODES[0].position), new Vector3(...NETWORK_NODES[0].position)])
