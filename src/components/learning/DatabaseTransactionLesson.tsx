...
const TRANSACTION_STEPS: TransactionStep[] = [
  {
    ...
    body: '送金処理を始める前に、アプリケーションはトランザクション（Transaction）を開始します。ここからコミット（Commit）またはロールバック（Rollback）までの一連の更新を、1つの仕事として扱います。',
    detail: 'この教材の読み取りコミット済み（Read Committed）相当の概念モデルでは、口座Aと口座Bの確定済み残高はまだ変わっていません。トランザクションは「複数の変更の途中だけが別の処理に見えてしまう」問題を避ける土台になります。',
  },
  {
    id: 'lock',
    label: 'LOCK',
    ...
    detail: 'ロックの形や待機の扱いはデータベース製品・分離レベル（Isolation Level）・実装によって異なります。ここでは排他ロックを使う代表例として、更新競合を見える化しています。',
  },
  {
    id: 'debit',
    label: 'UPDATE A',
    title: '口座Aの残高から300を引く（まだ確定しない）',
    body: 'トランザクションの作業中ビューでは、口座Aが1000から700になります。この教材の読み取りコミット済み相当のモデルでは、この時点で別の処理へ「Aだけ減った状態」を確定結果として見せません。',
    detail: '途中で障害や検証エラーが起きればロールバック（Rollback）で開始前の状態へ戻せます。トランザクションの途中の値をほかの処理がどう見られるかは分離レベル（Isolation Level）によって異なります。',
  },
  {
    id: 'credit',
    label: 'UPDATE B',
    title: '口座Bの残高へ300を加える（まだ確定しない）',
    body: '同じトランザクションの中で、口座Bは200から500になります。必要な更新がそろったので、次に全体を確定できる状態です。',
    detail: '送金額、残高不足、更新件数などの検証もコミット（Commit）前に行います。ここでは説明のため、残高不足や手数料などを省いた単純な例にしています。',
  },
  {
    id: 'commit',
    label: 'COMMIT',
    title: '2つの更新をまとめて確定し、ロックを解放する',
    body: 'コミット（Commit）後、この教材の確定済みビューはA=700、B=500になります。途中のA=700・B=200だけが確定するのではなく、成功した結果全体を扱えることがトランザクションの重要な性質です。',
    detail: '多くのデータベースは障害復旧を考慮して更新ログなどを利用します。どこまでを「確定」と扱うかの実装詳細や耐障害性は、製品・設定・ストレージ構成で変わります。',
  },
]
...
function AccountCard({
  ...
  return <article className={`rounded-2xl border p-4 ${colors}`}>
    <div className="flex items-start justify-between gap-3">
      <div><p className="text-[10px] font-bold tracking-[.14em] text-slate-500">口座</p><h4 className="mt-1 text-sm font-bold">{name}</h4></div>
...
export function DatabaseTransactionLesson({ onNavigate }: DatabaseTransactionLessonProps) {
  ...
  const status = rolledBack
    ? { label: 'ロールバック（ROLLBACK）済み', body: '途中の更新を破棄し、確定済み残高を開始前の状態へ戻しました。ロックも解放されます。', className: 'border-rose-300 bg-rose-50 text-rose-950' }
    : isCommitted
      ? { label: 'コミット（COMMIT）済み', body: '2つの更新がまとまって確定し、ほかの更新を待たせていたロックを解放しました。', className: 'border-emerald-300 bg-emerald-50 text-emerald-950' }
      : { label: '処理を一時停止中', body: 'いまの段階で止めて、作業中の値・外から見える確定値・ロックの役割を見比べられます。', className: 'border-cyan-200 bg-cyan-50 text-cyan-950' }

  return <section aria-label="データベースのトランザクションのステップ図解" className="rounded-3xl border border-indigo-200 bg-indigo-50/45 p-5 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <p className="eyebrow text-indigo-700">操作して学ぶデータベース</p>
        <h2 className="mt-2 text-xl font-bold text-slate-900">送金を途中で止めて、トランザクションの役割を読む</h2>
        <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-700"><LinkedText text="データベースのトランザクション（Transaction）は、複数の更新を「全部成功」か「全部取り消し」にまとめるための境界です。この例では、口座Aから口座Bへ300円を送る処理を、各段階で止めながら見ます。" onNavigate={onNavigate} /></p>
      </div>
    ...
    <div className="mt-6 grid gap-2 sm:grid-cols-5" role="tablist" aria-label="トランザクションの段階">
    ...
    <div className="mt-4 flex flex-wrap gap-2">
      ...
      <button type="button" onClick={rollBack} disabled={rolledBack || step < 2 || step >= TRANSACTION_STEPS.length - 1} className="rounded-lg border border-rose-300 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-800 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-45">ここで失敗したとしてロールバック</button>
    ...
    <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
      <div className="grid gap-4 lg:grid-cols-[1.05fr_.95fr]">
        <div aria-live="polite">
          <p className="text-xs font-bold text-indigo-800">{rolledBack ? 'ロールバック（ROLLBACK）' : current.label}</p>
          <h3 className="mt-1 text-lg font-bold text-slate-900">{rolledBack ? '途中の変更を確定させずに取り消す' : current.title}</h3>
          <p className="mt-2 text-sm leading-7 text-slate-700"><LinkedText text={rolledBack ? '入力値の検証に失敗した、途中で障害が起きた、残高が不足していたなどの場合、ロールバック（Rollback）を選べます。トランザクション内で行った変更は確定前なので、開始前の整合した状態へ戻せます。' : current.body} onNavigate={onNavigate} /></p>
          <p className="mt-3 rounded-lg border-l-2 border-amber-400 bg-amber-50 px-3 py-2 text-xs leading-6 text-amber-950"><LinkedText text={rolledBack ? 'ロールバック（Rollback）は、すでにコミット（Commit）された別のトランザクションを自動で取り消す操作ではありません。この例のように、まだコミットしていない自分のトランザクションを中止する代表例です。' : current.detail} onNavigate={onNavigate} /></p>
        </div>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[1fr_auto_1fr] xl:items-stretch">
        <section className={`rounded-2xl border p-4 ${isWorking ? 'border-indigo-300 bg-indigo-50/60' : 'border-slate-200 bg-slate-50'}`} aria-label="トランザクション内の作業中ビュー">
          <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-bold text-slate-900">トランザクション内の作業ビュー</p><p className="mt-1 text-xs text-slate-600">この処理だけが組み立てている途中の値（教育用の表現）</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${isWorking ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-600'}`}>{isWorking ? '更新中' : isCommitted ? '確定済み' : rolledBack ? '破棄済み' : '開始前'}</span></div>
        ...
        <div className="flex items-center justify-center" aria-hidden="true"><span className="rounded-full border border-indigo-200 bg-white px-3 py-2 text-xs font-bold text-indigo-700">コミット / ロールバック<br /><span className="font-normal text-slate-500">確定境界</span></span></div>
        <section className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4" aria-label="この教材での確定済みビュー">
          <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-bold text-slate-900">この教材での確定済みビュー</p><p className="mt-1 text-xs text-slate-600">読み取りコミット済み（Read Committed）相当の概念モデルで、別の処理へ示す確定結果</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${isCommitted ? 'bg-emerald-600 text-white' : 'bg-white text-emerald-800'}`}>{isCommitted ? '新しい結果' : '開始時の結果'}</span></div>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_.9fr]">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-sm font-bold text-slate-900">保存される記録（概念図）</p><ol className="mt-2 space-y-1.5 text-xs text-slate-600"><li className={step >= 0 ? 'font-semibold text-slate-800' : ''}>01. BEGIN 送金 A → B</li><li className={step >= 2 && !rolledBack ? 'font-semibold text-slate-800' : ''}>02. UPDATE 口座 A</li><li className={step >= 3 && !rolledBack ? 'font-semibold text-slate-800' : ''}>03. UPDATE 口座 B</li><li className={isCommitted ? 'font-bold text-emerald-700' : rolledBack ? 'font-bold text-rose-700' : 'text-slate-400'}>{isCommitted ? '04. COMMIT' : rolledBack ? '04. ROLLBACK' : '04. COMMIT または ROLLBACK を待つ'}</li></ol></section>
      </div>
    </div>

    <p className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-6 text-amber-950"><b>教材上の簡略化：</b>これは行ロックを使うRDBMSの代表的な送金例です。「確定済みビュー」は読み取りコミット済み（Read Committed）相当の教育用の見せ方であり、実際のデータベースではMVCC、分離レベル（Isolation Level）、ログ、制約、分散トランザクション、障害復旧などにより、途中の値の見え方や競合時の結果が変わります。トランザクションが常にすべての不整合を防ぐわけではなく、アプリケーション側の検証や適切な設計も必要です。</p>
  </section>
}
