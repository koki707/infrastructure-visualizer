...
function stageForPath(pathId: string) {
  if (pathId === 'network-foundations') {
    return {
      label: 'ステップ2: 通信を深掘りする',
      description: 'この教材の前に、3DシミュレーションでPCからウェブサーバーまでの流れを一度見ると、役割がつながりやすくなります。',
      orderNote: '3Dシミュレーションで見た通信を理解しやすい順番に並べています。実際の1回の通信の厳密な処理順ではありません。',
    }
  }

  return {
    label: 'ステップ3: 他のIT分野へ広げる',
    description: 'ウェブアクセスの全体像と通信の土台を確認した後に、PC・OS・サービスを支える仕組みへ進むルートです。',
    orderNote: 'この分野の関係をつかむための推奨順です。実際の処理が必ずこの順番で起きることを示すものではありません。',
  }
}
...
export function LearningPathNavigator({ topicId, onNavigate }: Props) {
  ...
  const stage = stageForPath(path.id)
  ...
  return <nav aria-label="おすすめの学習ルート" className="rounded-2xl border border-cyan-200 bg-cyan-50/60 p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div>
        <p className="eyebrow">おすすめの学習ルート</p>
        <h2 className="mt-2 text-lg font-bold text-slate-900">{path.title}</h2>
        <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-700">{path.description}</p>
      </div>
      <button type="button" onClick={() => onNavigate('/topics')} className="rounded-lg border border-cyan-300 bg-white px-3 py-2 text-xs font-bold text-cyan-800 transition hover:border-cyan-500 hover:bg-cyan-50">教材の一覧へ</button>
    </div>

    <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-cyan-200 bg-white/80 p-4">
      <div>
        <p className="text-xs font-bold text-cyan-900">{stage.label}</p>
        <p className="mt-1 max-w-3xl text-xs leading-6 text-slate-600">{stage.description}</p>
      </div>
      <button type="button" onClick={() => onNavigate('/visualizer')} className="shrink-0 rounded-lg border border-cyan-300 bg-cyan-50 px-3 py-2 text-xs font-bold text-cyan-800 transition hover:border-cyan-500 hover:bg-cyan-100">3Dで全体を見る</button>
    </div>

    <ol className="mt-5 flex gap-2 overflow-x-auto pb-1" aria-label={`${path.title}の順番`}>
      {topics.map((item, index) => <li key={item.id} className="flex shrink-0 items-center gap-2">
        ...
        >
          <span className={`mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${item.id === topicId ? 'bg-white/20 text-white' : 'bg-cyan-50 text-cyan-800'}`}>{index + 1}</span>
          {item.shortTitle}
        </button>
    ...
    {currentIndex >= 0 && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-cyan-200 pt-4">
      <p className="text-xs leading-6 text-slate-600">このルートでは <b className="text-slate-900">{currentIndex + 1} / {topics.length}</b>。{stage.orderNote}</p>
