...
const TLS_STEPS: TlsStep[] = [
  {
    ...
    body: 'ブラウザは接続先の名前と、利用できるTLSの版・暗号スイート・鍵交換に関する情報などをサーバー（Server）へ送ります。これがTLS接続を始める代表的な最初のメッセージです。',
    detail: 'この時点ではHTTPリクエスト（HTTP Request）の本文はまだ保護されていません。ブラウザは、どの安全な組み合わせで通信できるかをサーバーとすり合わせようとしています。',
    clientLabel: '接続条件を提示',
    serverLabel: '条件を待つ',
  ...
  {
    shortTitle: 'サーバーハロー / 証明書',
    title: 'サーバーが選んだ条件と証明書を返す',
    body: 'サーバーは選んだ通信条件を知らせ、通常は自分の証明書を送ります。証明書には接続先の名前に関する情報と公開鍵などが含まれ、ブラウザが「想定した相手か」を確認する手がかりになります。',
    detail: '証明書は単なる身分証ではありません。信頼できる発行者からの署名、対象の名前、有効期間などを、ブラウザが複数の観点から確認します。',
    clientLabel: '証明書を受け取る',
    serverLabel: '条件と証明書を送る',
    direction: 'server-to-client',
    exchange: ['選択されたTLSの条件', '証明書チェーン（代表例）', 'サーバーの公開鍵に関する情報'],
    protected: '認証の材料を確認中',
  },
  {
    shortTitle: '証明書を検証',
    ...
    detail: '公開鍵は誰でも参照できる情報です。秘密に保つ必要があるのは対応する秘密鍵で、サーバーは秘密鍵を持つことをプロトコル上で証明します。',
    clientLabel: '証明書を検証',
    serverLabel: '秘密鍵を保持',
    direction: 'both',
  ...
  },
  {
    ...
    body: '合意した鍵交換方式を使い、ブラウザとサーバーは共有秘密を導出します。そこから、以降の通信を保護する対称鍵（ここではセッション鍵と呼びます）をそれぞれが計算します。',
  ...
  },
  {
    ...
    body: 'ここからHTTPリクエスト（HTTP Request）やHTTPレスポンス（HTTP Response）はTLSで保護されます。暗号化は内容を読まれにくくし、完全性の確認は途中で変更されていないかを確かめる助けになります。',
    detail: 'HTTPSは安全性に役立ちますが、Webサイト全体の安全性を単独で保証するものではありません。サーバー側の実装、ブラウザの検証、利用者の操作なども重要です。',
    clientLabel: '暗号化したHTTPリクエスト',
    serverLabel: '暗号化したHTTPレスポンス',
    direction: 'both',
    exchange: ['暗号化されたHTTPリクエスト', '暗号化されたHTTPレスポンス', '改ざんを検出するための情報'],
...
export function TlsHandshakeLesson({ onNavigate }: TlsHandshakeLessonProps) {
  ...
  return <section aria-label="TLSハンドシェイクのステップ図解" className="rounded-3xl border border-violet-200 bg-violet-50/50 p-5 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <p className="eyebrow text-violet-700">操作して学ぶTLS</p>
