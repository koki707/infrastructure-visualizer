...
const DNS_STEPS: DnsStep[] = [
  {
    ...
    detail: 'DNSが主に扱うのはホスト名です。/guide のようなPathや ?topic=network のようなQueryは、通常DNS Queryには含まれません。',
  },
  {
    shortTitle: 'DNS Query',
    title: 'PCは設定済みのDNSリゾルバへ問い合わせる',
    body: 'PCは、DHCPや手動設定などで知っているDNSリゾルバへ、docs.example.test のAレコードを問い合わせる代表例を示します。',
    detail: 'PCが直接Root DNS Serverへ問い合わせるわけではありません。この教材のDNS Serverは、問い合わせを受け取る再帰リゾルバを代表しています。',
  },
  {
    shortTitle: 'Cache確認',
  ...
  },
  {
    ...
    body: 'キャッシュに回答がなければ、リゾルバはフォワーダーや権威DNSなど、設定された上流の仕組みを使って情報を得ます。',
    detail: 'Root・TLD・権威DNSの詳しい往復、CNAME連鎖、DNSSEC検証などは、この最初の教材では省略しています。',
  },
  {
    shortTitle: 'DNS Answer',
    title: '接続先の情報をPCへ返し、次の通信へ進む',
    body: 'DNSリゾルバは回答をキャッシュし、PCへDNS Answerを返します。PCは得たIPアドレスを手がかりに、次にWeb Serverへの通信を始めます。',
    detail: 'DNS Answerを受け取っただけでは、TCP接続やHTTP通信は完了していません。これはWebアクセスの前段にある名前解決です。',
  },
]
...
export function DnsLesson({ onNavigate }: DnsLessonProps) {
  ...
  const dynamicBody = step === 3 && cacheHit
    ? '有効な回答がキャッシュにあれば、DNSリゾルバは上流DNSへ問い合わせず、保存済みの情報からDNS Answerを準備できます。'
    : current.body
  ...
  const sceneSummary = [
    ...
    { signal: 'DNS Query', focus: 'PC → Home Router', result: '設定済みResolverへ向かいます' },
    { signal: 'DNS Query', focus: 'Home Router → DNS Resolver', result: 'Resolverがキャッシュを確認します' },
    cacheHit
      ? { signal: 'Resolver Cache', focus: '有効な回答を再利用する', result: '上流DNSへの問い合わせを省略します' }
      : { signal: 'Upstream Query', focus: 'Resolverが必要な情報を上流へ問い合わせる', result: '回答を得てCacheへ保存します' },
    { signal: 'DNS Answer', focus: 'DNS Resolver → Home Router → PC', result: '203.0.113.10 を受け取ります' },
  ] as const
  ...
  return <section aria-label="DNSの3Dステップシミュレーション" className="rounded-3xl border border-indigo-200 bg-indigo-50/40 p-5 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="eyebrow text-indigo-700">INTERACTIVE DNS</p><h2 className="mt-2 text-xl font-bold text-slate-900">名前をResolverへ尋ね、接続先の情報を得る</h2><p className="mt-2 max-w-3xl text-sm leading-7 text-slate-700"><LinkedText text="DNS QueryとDNS Answerの移動、Resolver Cache、必要なときだけ行う上流への問い合わせを、段階を止めながら確かめます。" onNavigate={onNavigate} /></p></div>
    ...
    <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-indigo-200 bg-white p-4">
      <div><p className="text-sm font-bold text-slate-900">Resolver Cacheの状態</p><p className="mt-1 text-xs leading-6 text-slate-600">同じ名前でも、キャッシュが使えるかで上流への問い合わせの有無が変わります。</p></div>
      <div className="flex rounded-xl border border-indigo-200 bg-indigo-50 p-1" role="group" aria-label="Resolver Cacheの状態を選択">
    ...
    <section className="mt-6 overflow-hidden rounded-2xl border border-indigo-200 bg-white" aria-label="DNSの3Dシミュレーション">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-indigo-100 bg-indigo-50 px-4 py-3">
        <div><p className="eyebrow text-indigo-700">3D DNS SIMULATION</p><p className="mt-1 text-sm font-bold text-slate-900">{currentScene.signal}</p></div>
    ...
    <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6" aria-live="polite">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-3">
        <div className={`rounded-xl border p-3 ${step === 1 || step === 2 ? 'border-indigo-300 bg-indigo-50' : 'border-slate-200 bg-slate-50'}`}><p className="text-xs font-bold text-slate-700">DNS Query（例）</p><code className="mt-2 block break-words text-xs leading-6 text-indigo-950">QNAME: docs.example.test<br />QTYPE: A</code></div>
        <div className={`rounded-xl border p-3 ${step >= 2 ? cacheHit ? 'border-emerald-300 bg-emerald-50' : 'border-amber-300 bg-amber-50' : 'border-slate-200 bg-slate-50'}`}><p className="text-xs font-bold text-slate-700">Resolver Cache（例）</p><code className="mt-2 block break-words text-xs leading-6 text-slate-800">{cacheHit ? 'docs.example.test → 203.0.113.10\nTTL: 300 seconds' : step >= 3 ? 'Cache miss → answer stored\nTTL: 300 seconds' : 'まだ回答を確認していません'}</code></div>
        <div className={`rounded-xl border p-3 ${step === 4 ? 'border-emerald-300 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}><p className="text-xs font-bold text-slate-700">DNS Answer（例）</p><code className="mt-2 block break-words text-xs leading-6 text-emerald-900">docs.example.test<br />A 203.0.113.10</code></div>
      </div>
    </section>
