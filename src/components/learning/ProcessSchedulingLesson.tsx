...
const SCHEDULING_STEPS: SchedulingStep[] = [
  {
    shortTitle: '実行待ち',
    title: '実行したい仕事が、Ready Queueで待っている',
    body: 'Process AとProcess Bは、CPUを使える状態ですが、同時には実行できません。SchedulerはReady状態の候補から、次にCPUを使う実行単位を選びます。',
    detail: 'ここでは状態を見やすくするため、Process A/Bを実行単位として表示します。多くのOSではSchedulerが直接選ぶのはThreadです。複数コアがあれば複数の実行単位が同時にRunningになれますが、それぞれのコアごとに選択が必要です。',
    processA: 'ready',
    processB: 'ready',
    cpuOwner: 'none',
    event: 'Schedulerが候補を選ぶ',
    savedContext: 'まだ切り替え前',
  },
  {
    shortTitle: 'Aを実行',
    title: 'SchedulerがProcess Aを選び、CPUで実行する',
    body: 'SchedulerがProcess Aを選ぶと、Aの実行に必要な状態をCPUへ戻し、AがRunningになります。Process BはReady Queueで待ち続けます。',
    detail: '実行に必要な状態には、次に実行する位置を表すProgram Counter、Registerの値、Stack Pointerなどが含まれます。何を保存するかはCPUの仕様やOSの実装で異なります。',
    processA: 'running',
    processB: 'ready',
    cpuOwner: 'A',
    event: 'AのContextを復元 → CPUで実行',
    savedContext: 'A: PC=0x1204 / R1=7',
  },
  {
    shortTitle: '切り替え',
    title: '時間経過などをきっかけに、Aの状態を保存する',
    body: 'タイムスライスの終了や割り込みなどをきっかけに、OSはAの実行をいったん止めます。AのContextを保存してReady状態へ戻し、別の候補を実行できるようにします。',
    detail: 'Context Switchには時間がかかります。切り替え中はアプリケーションの仕事が進まないため、OSは応答性と切り替えコストのバランスを考えます。',
    processA: 'ready',
    processB: 'ready',
    cpuOwner: 'none',
    event: 'AのContextを保存 → Ready Queueへ',
    savedContext: 'A: PC=0x1204 / R1=7 を保存',
  },
  {
    shortTitle: 'Bを実行',
    title: 'Process Bの状態を復元して、CPUを渡す',
    body: 'Schedulerは次の候補としてProcess Bを選びます。BのContextを復元すると、CPUはBが前回止まった位置から処理を続けられます。',
    detail: '見かけ上は複数のプログラムが同時に動いているように見えても、1コアでは短い時間ごとに実行対象を切り替えている場合があります。',
    processA: 'ready',
    processB: 'running',
    cpuOwner: 'B',
    event: 'BのContextを復元 → CPUで実行',
    savedContext: 'B: PC=0x0810 / R1=42 を復元',
  },
  {
    shortTitle: 'I/O待ち',
    title: 'BがI/Oを待つ間、CPUは別の仕事へ回せる',
    body: 'BがディスクやネットワークなどのI/O完了を待つ必要があると、BはWaiting状態になります。SchedulerはCPUを空けず、Ready状態のAを選んで実行できます。',
    detail: 'I/O完了の通知は割り込みなどを通じてOSへ届くことがあります。WaitingからReadyへ戻る時点と、すぐにCPUを得られるかは、ほかの候補やスケジューリング方針によって変わります。',
    processA: 'running',
    processB: 'waiting',
    cpuOwner: 'A',
    event: 'BがWaitingへ → Aを再び実行',
...
function ProcessCard({ name, state, active, context }: { name: string; state: ProcessState; active: boolean; context: string }) {
  return <article className={`rounded-xl border p-4 transition duration-200 ${active ? 'border-emerald-400 bg-emerald-50 shadow-sm shadow-emerald-100' : 'border-slate-200 bg-white'}`}>
    <div className="flex items-start justify-between gap-2"><div><p className="text-[10px] font-bold tracking-wide text-slate-500">PROCESS</p><h4 className="mt-1 text-base font-bold text-slate-900">Process {name}</h4></div><span className={`rounded-full border px-2 py-1 text-[10px] font-bold ${STATE_STYLE[state]}`}>{STATE_LABEL[state]}</span></div>
    <div className="mt-4 rounded-lg bg-slate-900 px-3 py-2 font-mono text-[11px] text-slate-100"><span className="text-slate-400">Context</span><br />{context}</div>
  </article>
}
...
export function ProcessSchedulingLesson({ onNavigate }: ProcessSchedulingLessonProps) {
  ...
  return <section aria-label="OSのスケジューリングとコンテキストスイッチのステップ図解" className="rounded-3xl border border-indigo-200 bg-indigo-50/45 p-5 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <p className="eyebrow text-indigo-700">INTERACTIVE OS</p>
        <h2 className="mt-2 text-xl font-bold text-slate-900">CPUを、複数の仕事へどう割り当てるか</h2>
        <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-700"><LinkedText text="この教材では説明のためProcess AとProcess Bを実行単位として表示し、1つのCPUコアで動かします。多くのOSではSchedulerはThreadを選びます。Ready・Running・Waitingの状態、Scheduler、Context Switchを止めながら確認します。" onNavigate={onNavigate} /></p>
      </div>
    ...
    <div id="schedule-step-panel" role="tabpanel" aria-labelledby={`schedule-step-${step}`} className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
        ...
        <aside className="rounded-xl border border-indigo-200 bg-indigo-50 p-4"><p className="text-xs font-bold text-indigo-900">保存・復元する状態（例）</p><code className="mt-3 block rounded-lg bg-white px-3 py-2 font-mono text-xs font-bold leading-6 text-slate-800">{current.savedContext}</code><p className="mt-3 text-xs leading-6 text-slate-600">このような実行途中の情報を保存して別の実行単位へ切り替えることを、Context Switchと呼びます。</p></aside>
      </div>

      <section className="mt-5 grid gap-3 md:grid-cols-3" aria-label="プロセス状態の意味"><article className="rounded-xl border border-sky-200 bg-sky-50 p-3"><p className="text-xs font-bold text-sky-900">Ready</p><p className="mt-1 text-xs leading-5 text-slate-700">CPUを使えるが、順番を待っている状態。</p></article><article className="rounded-xl border border-emerald-200 bg-emerald-50 p-3"><p className="text-xs font-bold text-emerald-900">Running</p><p className="mt-1 text-xs leading-5 text-slate-700">選ばれてCPU上で命令を実行している状態。</p></article><article className="rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-bold text-amber-900">Waiting</p><p className="mt-1 text-xs leading-5 text-slate-700">I/Oなど、外部の完了を待ってCPUを譲っている状態。</p></article></section>
    </div>

    <section className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm font-bold text-slate-900">ProcessとThreadを分けて考える</p><p className="mt-2 text-sm leading-7 text-slate-700">Processは、独立した仮想アドレス空間や資源を持つプログラムの実行単位です。Threadは、そのProcess内で実際に命令を実行する流れです。多くのOSではSchedulerが直接選ぶ対象はThreadですが、この教材では状態の変化を見やすくするためProcessを表にしています。</p></section>
    <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-6 text-amber-950"><b>教材上の簡略化：</b>スケジューリング方針はRound Robinだけではなく、優先度、対話性、CPUコア数、OSの設計などで変わります。ここでは「実行中の状態を保存し、次の仕事を復元する」という中心的な役割を示しています。</p>
  </section>
}
