...
function phaseTitle(phase: TraversalPhase, algorithm: Algorithm) {
  ...
  if (phase === 'take') return algorithm === 'bfs' ? 'Queueの先頭から次のNodeを取り出す' : 'Stackの上から次のNodeを取り出す'
  if (phase === 'expand') return '隣接する未発見Nodeを追加する'
  if (phase === 'found') return '目的のNodeを見つけた'
  return '探索できるNodeをすべて調べた'
}
...
export function GraphTraversalLesson({ onNavigate }: GraphTraversalLessonProps) {
  ...
  const advance = () => {
    if (phase === 'ready') {
      ...
      setDiscovered([start])
      setPhase('take')
      setLog([`${start} を開始点として、${algorithm === 'bfs' ? 'Queue' : 'Stack'}へ追加しました。`])
      return
    }
    ...
    if (phase === 'take') {
      if (frontier.length === 0) {
        setPhase('complete')
        setLog(items => [...items, '探索できるNodeがなくなりました。'])
        return
      }
      ...
      if (nextCurrent === TARGET) {
        ...
        setPhase('expand')
        setLog(items => [...items, `${nextCurrent} を取り出し、隣接するNodeを調べます。`])
      }
      return
    }
    ...
    if (phase === 'expand' && current) {
      ...
      setCurrent(null)
      setPhase('take')
      setLog(items => [...items, newNodes.length > 0 ? `${current} の未発見の隣接Node ${newNodes.join('・')} を${algorithm === 'bfs' ? 'Queueの末尾' : 'Stackの上'}へ追加しました。` : `${current} から追加する未発見Nodeはありません。`])
    }
  }

  const frontierName = algorithm === 'bfs' ? 'Queue（先入れ先出し）' : 'Stack（後入れ先出し）'
  const canAdvance = phase !== 'found' && phase !== 'complete'
  const statusBody = phase === 'ready'
    ? `${start} から ${TARGET} を探します。${algorithm === 'bfs' ? '近いNodeから順に広げる' : '1つの枝を深くたどる'}動きを、1手ずつ止めて確認できます。`
    : phase === 'take'
      ? `${frontierName}には、次に調べる候補が並んでいます。`
      : phase === 'expand' && current
        ? `${current} とつながる未発見Nodeを候補へ追加します。同じNodeを二重に入れないため、発見済みとして記録します。`
        : phase === 'found'
          ? `${TARGET} に到達したため、この検索例では停止します。`
          : '開始点から到達できるNodeをすべて調べ終えました。'
  ...
  return <section aria-label="BFSとDFSのグラフ探索図解" className="rounded-3xl border border-cyan-200 bg-cyan-50/45 p-5 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <h2 className="mt-2 text-xl font-bold text-slate-900">Nodeをたどり、目的地を探す</h2>
        <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-700"><LinkedText text="Graph Traversalは、NodeとEdgeで表したつながりを順にたどる探索です。BFSとDFSを切り替え、QueueまたはStackが探索順をどう変えるかを止めながら確かめます。" onNavigate={onNavigate} /></p>
      </div>
      <span className="rounded-full border border-cyan-200 bg-white px-3 py-1.5 text-xs font-bold text-cyan-800">目標 Node: {TARGET}</span>
    </div>

    <div className="mt-6 grid gap-3 lg:grid-cols-2">
      <div className="rounded-2xl border border-cyan-200 bg-white p-4"><p className="text-sm font-bold text-slate-900">探索方法を選ぶ</p><div className="mt-3 flex flex-wrap gap-2" role="group" aria-label="グラフ探索の方法">{(['bfs', 'dfs'] as Algorithm[]).map(item => <button key={item} type="button" aria-pressed={algorithm === item} onClick={() => selectAlgorithm(item)} className={`rounded-lg border px-3 py-2 text-xs font-bold transition ${algorithm === item ? 'border-cyan-600 bg-cyan-700 text-white' : 'border-slate-200 bg-white text-slate-700 hover:border-cyan-300 hover:text-cyan-800'}`}>{item === 'bfs' ? 'BFS：幅を広げる' : 'DFS：深くたどる'}</button>)}</div><p className="mt-3 text-xs leading-6 text-slate-600"><LinkedText text={algorithm === 'bfs' ? 'BFSはQueueを使い、開始点から近いNodeを先に調べます。' : 'DFSはStackを使い、見つけた枝を深くたどってから戻ります。'} onNavigate={onNavigate} /></p></div>
      <div className="rounded-2xl border border-cyan-200 bg-white p-4"><p className="text-sm font-bold text-slate-900">開始するNodeを選ぶ</p><div className="mt-3 flex flex-wrap gap-2" role="group" aria-label="グラフ探索の開始Node">{START_NODES.map(node => <button key={node} type="button" aria-pressed={start === node} onClick={() => selectStart(node)} className={`rounded-lg border px-3 py-2 text-xs font-bold transition ${start === node ? 'border-cyan-600 bg-cyan-700 text-white' : 'border-slate-200 bg-white text-slate-700 hover:border-cyan-300 hover:text-cyan-800'}`}>{node} から始める</button>)}</div><p className="mt-3 text-xs leading-6 text-slate-600">開始点を変えると、候補の並びと目的地へたどり着く順序が変わります。</p></div>
    </div>

    <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
      <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        <div>
          <div className="relative min-h-[318px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-50" aria-label="グラフのNodeとEdge">
        ...
        <aside className="space-y-3">
          ...
          <section className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4"><p className="text-xs font-bold text-emerald-900">発見済みNode</p><p className="mt-2 min-h-6 font-mono text-sm font-bold text-emerald-950">{discovered.length ? discovered.join(' → ') : '—'}</p><p className="mt-2 text-[11px] leading-5 text-slate-600">同じNodeを候補へ重ねて入れないために使います。</p></section>
          <section className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-bold text-slate-700">展開済みNode</p><p className="mt-2 min-h-6 font-mono text-sm font-bold text-slate-800">{expanded.length ? expanded.join(' → ') : '—'}</p><p className="mt-2 text-[11px] leading-5 text-slate-600">隣接Nodeを調べ終えたNodeです。</p></section>
        </aside>
      </div>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2"><p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-6 text-amber-950"><b>最短経路との関係：</b>BFSは、すべてのEdgeを同じ重みとして扱う無重みGraphで、開始点からの最短の「辺の数」を求めるときに使えます。重みが異なる場合は、別のアルゴリズムが必要になることがあります。</p><p className="rounded-xl border border-cyan-200 bg-white px-4 py-3 text-xs leading-6 text-slate-700"><b>DFSとの違い：</b>DFSは1つの枝を深くたどるため、探索順がBFSと異なり、最短経路を保証する探索ではありません。どちらが適切かは、探索の目的とGraphの性質で変わります。</p></div>
  </section>
}
