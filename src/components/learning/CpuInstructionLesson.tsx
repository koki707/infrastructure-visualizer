...
type InstructionStep = {
  shortTitle: 'フェッチ（Fetch）' | 'デコード（Decode）' | '実行（Execute）' | '書き戻し（Writeback）'
  title: string
  body: string
  detail: string
...
const INSTRUCTION_STEPS: InstructionStep[] = [
  {
    shortTitle: 'フェッチ（Fetch）',
    title: '次に実行する命令をメモリから取り出す',
    body: 'プログラムカウンタ（PC）が指す番地を使ってメモリを読み、命令を命令レジスタ（IR）へ入れます。この例では、番地 0x0040 にある ADD R1, R2, R3 を取り出します。',
    detail: 'PCは「次に読む命令の位置」を表すためのレジスタです。この例では説明のため4バイト固定長命令のモデルとして、PCを 0x0040 から 0x0044 へ進めています。実際にどのように次の位置を決めるかは、命令セットや実装によって異なります。',
  ...
  {
    shortTitle: 'デコード（Decode）',
    title: '命令の意味と必要な入力を読み解く',
    body: '制御装置（Control Unit）はIRのビット列を解釈し、この命令が加算であること、結果をR1へ書くこと、R2とR3の値が必要であることを決めます。',
    detail: 'デコード（Decode）は人が文字列を読む処理ではありません。CPUは命令セットで定められたビットの並びを回路で判別し、制御信号を作ります。',
  ...
  {
    shortTitle: '実行（Execute）',
    title: 'ALU（算術論理演算装置）が指定された演算を行う',
    body: 'レジスタから渡された R2=7 と R3=5 をALUが加算します。演算の結果は 12 です。この段階では、結果がまだR1へ確定していないモデルとして示しています。',
    ...
    r3: '5 → ALU',
    alu: '7 + 5 = 12',
    signal: 'R2 / R3 → ALU（算術論理演算装置）',
  },
  {
    shortTitle: '書き戻し（Writeback）',
    title: '演算結果を行き先のレジスタへ書き戻す',
    body: 'ALUの結果 12 をR1へ書き戻します。これで、このADD命令の結果を後続の命令が利用できる状態になります。',
    detail: '「書き戻し（Writeback）」という名前の段階を明確に持つか、どの時点で結果を見えるようにするかはCPUの設計で変わります。ここではデータの流れを分けて理解するために表示しています。',
...
export function CpuInstructionLesson({ onNavigate }: CpuInstructionLessonProps) {
  ...
  return <section aria-label="CPU命令実行のステップ図解" className="rounded-3xl border border-cyan-200 bg-cyan-50/45 p-5 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <p className="eyebrow">操作して学ぶCPU</p>
        <h2 className="mt-2 text-xl font-bold text-slate-900">1つの命令を、止めながらCPUの中で追う</h2>
        <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-700"><LinkedText text="ADD R1, R2, R3 という1つの命令を例に、メモリ、レジスタ、制御装置（Control Unit）、ALUの間で値がどう動くかを段階ごとに確認します。" onNavigate={onNavigate} /></p>
      </div>
    ...
    <div id="cpu-step-panel" role="tabpanel" aria-labelledby={`cpu-step-${step}`} className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_1.15fr_1fr]">
        <PartCard title="命令メモリ" subtitle="プログラムを読む" active={isActive('memory')}><div className="rounded-lg bg-slate-900 px-3 py-2 font-mono text-xs text-slate-100"><span className="text-cyan-300">0x0040</span>  ADD R1, R2, R3</div><p className="mt-2 text-xs leading-5 text-slate-600">この例では、命令がここにあると考えます。</p></PartCard>
        <PartCard title="制御装置（Control Unit）" subtitle="命令を解釈して制御する" active={isActive('control')}><div className="grid gap-2 sm:grid-cols-2"><div className={`rounded-lg px-3 py-2 text-xs font-semibold ${isActive('pc') ? 'bg-cyan-100 text-cyan-950' : 'bg-slate-100 text-slate-700'}`}>プログラムカウンタ（PC）<br /><code className="font-mono">{current.pc}</code></div><div className={`rounded-lg px-3 py-2 text-xs font-semibold ${isActive('ir') ? 'bg-cyan-100 text-cyan-950' : 'bg-slate-100 text-slate-700'}`}>命令レジスタ（IR）<br /><code className="font-mono text-[10px]">{current.ir}</code></div></div><p className="mt-2 text-xs leading-5 text-slate-600">命令に応じて、どのデータ経路を使うかを制御します。</p></PartCard>
        <PartCard title="ALU（算術論理演算装置）" subtitle="演算を行う" active={isActive('alu')}><div className={`rounded-lg px-3 py-2 font-mono text-sm font-bold ${isActive('alu') ? 'bg-violet-100 text-violet-950' : 'bg-slate-100 text-slate-600'}`}>{current.alu}</div><p className="mt-2 text-xs leading-5 text-slate-600"><LinkedText text="ALUは加算・比較・論理演算などを担当します。" onNavigate={onNavigate} /></p></PartCard>
      </div>

      <PartCard title="レジスタ（Registers）" subtitle="すぐに使う値を保持する小さな記憶領域" active={isActive('registers')}><div className="grid gap-2 sm:grid-cols-3"><div className={`rounded-lg border px-3 py-2 ${current.r1.includes('12') ? 'border-emerald-300 bg-emerald-50' : 'border-slate-200 bg-white'}`}><p className="text-[10px] font-bold text-slate-500">R1（書き込み先）</p><code className="mt-1 block font-mono text-sm font-bold text-slate-900">{current.r1}</code></div><div className="rounded-lg border border-slate-200 bg-white px-3 py-2"><p className="text-[10px] font-bold text-slate-500">R2</p><code className="mt-1 block font-mono text-sm font-bold text-slate-900">{current.r2}</code></div><div className="rounded-lg border border-slate-200 bg-white px-3 py-2"><p className="text-[10px] font-bold text-slate-500">R3</p><code className="mt-1 block font-mono text-sm font-bold text-slate-900">{current.r3}</code></div></div></PartCard>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
        ...
        <aside className="rounded-xl border border-violet-200 bg-violet-50 p-4"><p className="text-xs font-bold text-violet-900">この段階で見るポイント</p><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-700"><li><span className="font-bold text-violet-800">1.</span> 命令はメモリから取り出す</li><li><span className="font-bold text-violet-800">2.</span> 制御装置がデータ経路を選ぶ</li><li><span className="font-bold text-violet-800">3.</span> 結果はレジスタへ戻る</li></ul></aside>
      </div>
    </div>

    <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-6 text-amber-950"><b>教材上の簡略化：</b>ここでは理解のため、1命令をフェッチ → デコード → 実行 → 書き戻しと順番に止めています。実際のCPUでは命令セット、キャッシュ（Cache）、パイプライン（Pipeline）、分岐予測、複数の演算器などによって、複数命令の処理が重なったり順序が変わったりします。</p>
  </section>
}
